...
try:
    ...
    # Handle Pillow version >= 10.0.0 compatibility where LANCZOS was moved.
    if not hasattr(Image, 'LANCZOS'):
        try:
            Image.LANCZOS = Image.Resampling.LANCZOS
        except AttributeError:
            # Fallback for very old versions, though less likely.
...
def save_last_paths_checkpoint():
    ...
    try:
        ...
        payload = {
            ...
            "ortho4xp_imagery": safe_get(ortho_imagery_var), "ortho4xp_zl": safe_get(ortho_zl_var),
            "ortho4xp_dem_choice": safe_get(ortho_dem_choice_var), "ortho4xp_dem_path": safe_get(ortho_dem_custom_path_var),
            "ortho4xp_override_step1": safe_get_bool(ortho_override_step1_var), "ortho4xp_apply_defaults": safe_get_bool(ortho_apply_defaults_var),
...
def load_startup_preferences():
    ...
    try:
        ...
        if "ortho4xp_override_step1" in state: ortho_override_step1_var.set(bool(state["ortho4xp_override_step1"]))
        if "ortho4xp_apply_defaults" in state: ortho_apply_defaults_var.set(bool(state["ortho4xp_apply_defaults"]))
...
def map_lonlat_to_xy(lon, lat, w, h):
    view = map_view_state
    x = (lon - view["lon_min"]) / (view["lon_max"] - view["lon_min"]) * w
    y = (view["lat_max"] - lat) / (view["lat_max"] - view["lat_min"]) * h
    return x, y

def map_xy_to_lonlat(x, y, w, h):
    view = map_view_state
    lon = view["lon_min"] + (x / max(w, 1)) * (view["lon_max"] - view["lon_min"])
    lat = view["lat_max"] - (y / max(h, 1)) * (view["lat_max"] - view["lat_min"])
    return lon, lat
...
def build_map_background_image(canvas_w, canvas_h, view):
    ...
    comp_lon_min, comp_lat_max = tile_xy_to_lonlat(tile_x_min, tile_y_min, zoom)
    comp_lon_max, comp_lat_min = tile_xy_to_lonlat(tile_x_max + 1, tile_y_max + 1, zoom)
    comp_w_px, comp_h_px = composite.size
    def lon_to_px(lon): return (lon - comp_lon_min) / (comp_lon_max - comp_lon_min) * comp_w_px
    def lat_to_py(lat): return (comp_lat_max - lat) / (comp_lat_max - comp_lat_min) * comp_h_px
    cx1, cx2 = sorted([lon_to_px(view["lon_min"]), lon_to_px(view["lon_max"])])
    cy1, cy2 = sorted([lat_to_py(view["lat_max"]), lat_to_py(view["lat_min"])])
...
def open_map_window():
    ...
    ttk.Label(toolbar, text="Bright Green=OSM Session | Dark Green=Ortho ZL | Light Green=OSM Output | Purple=X-Plane", font=("Arial", 8)).pack(side='left', padx=10)
...
def download_ot_dem(lat, lon, dem_type, api_key, cache_dir):
    """Downloads a DEM from OpenTopography for a given tile using PowerShell."""
    if not api_key:
    ...
    if sys.platform != "win32":
        return None, "Automatic DEM download via PowerShell is only supported on Windows."

    ps_command = (
        f'$apiKey="{api_key}"; '
        f'Invoke-RestMethod -Uri "https://portal.opentopography.org/API/globaldem?demtype={dem_type}'
        f'&south={lat}&north={lat+1}&west={lon}&east={lon+1}'
        f'&outputFormat=GTiff&API_Key=$apiKey" -OutFile \'{dem_path}\''
    )
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            if "401" in error_message and "Unauthorized" in error_message:
                 return None, "PowerShell download failed: HTTP Error 401 (Unauthorized). Please double-check your API key."
            return None, f"PowerShell download failed with error:\n{error_message}"

        if not os.path.exists(dem_path) or os.path.getsize(dem_path) < 1000:
            return None, "PowerShell command ran, but the downloaded file is missing or empty. Check for network or permission issues."

        return dem_path, "Downloaded successfully via PowerShell."

    except FileNotFoundError:
        return None, "PowerShell is not installed or not in the system's PATH."
    except subprocess.TimeoutExpired:
        return None, "The PowerShell download command timed out after 120 seconds."
    except Exception as e:
        return None, f"An unexpected error occurred when calling PowerShell: {e}"
...
def read_ortho4xp_hierarchical_cfg_value(ortho_dir, key):
    ...
    cfg_paths = [
        # For standalone Ortho4XP v1.3.0+, this is the correct global config file.
        # We target only this file to avoid ambiguity with other possible cfg files.
        os.path.join(ortho_dir, "_internal", "Ortho4XP_Data", "Ortho4XP.cfg")
    ]

    for cfg_path in cfg_paths:
        value = read_ortho4xp_cfg_value(cfg_path, key)
...
def sync_ortho4xp_defaults_from_cfg():
    ...
    if not ortho4xp_output_var.get().strip():
        ortho4xp_output_var.set(scenery_dir if scenery_dir else os.path.join(ortho_dir, "Tiles"))
...
def write_ortho4xp_main_panel_settings(imagery=None, zl=None, scenery_dir=None, custom_dem=None, custom_build_dir=None, dem_source=None, ot_api_key=None):
    ortho_dir = get_ortho4xp_dir()
    if not ortho_dir:
        return False, "Ortho4XP.exe path is not set."

    cfg_path = os.path.join(ortho_dir, "_internal", "Ortho4XP_Data", "Ortho4XP.cfg")

    # 1. Read existing config into a dictionary
    current_cfg_dict = {}
        ...
        try:
            with open(cfg_path, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    ...
                    current_cfg_dict[key.strip()] = value.strip().strip("'\"")
        except Exception as e:
            # If reading fails, proceed with an empty dict, but log the error
            print(f"Warning: Could not read existing Ortho4XP.cfg: {e}")

    # 2. Update specific keys with new values, handling special cases
    ...
    try:
        ...
        return False, f"Could not write {os.path.basename(cfg_path)}:\n{e}"
...
def cleanup_selected_tile_batch():
    ...
    if map_selection_bounds:
        S_LAT, E_LAT, S_LON, E_LON = map_selection_bounds
    else:
        try:
            ...
            messagebox.showerror("Format Error", "Coordinates parameters must use whole integers only!")
            return
    if S_LAT > E_LAT or S_LON > E_LON:
        messagebox.showerror("Logic Error", "Starting coordinates must be lower than ending limits!")
        return
    tile_folders_to_remove = [f"zOrtho4XP_{_ortho4xp_tile_id(lat, lon)}" for lat in range(S_LAT, E_LAT + 1) for lon in range(S_LON, E_LON + 1)]
    tile_ids_to_remove = [_ortho4xp_tile_id(lat, lon) for lat in range(S_LAT, E_LAT + 1) for lon in range(S_LON, E_LON + 1)]
    if not tile_folders_to_remove:
...
def trigger_ortho4xp_stop():
    global ortho4xp_stop_requested
    if messagebox.askyesno("Stop Ortho4XP Batch?", "Stop the batch before the next tile starts building?"):
        ortho4xp_stop_requested = True
        ortho_status_var.set("Status: Stopping before the next tile starts...")

def _restore_ortho_panel():
    ortho_progress_frame.pack_forget()
    input_frame.pack(fill='x', expand=True, padx=10, pady=5)
    ortho_frame.pack(fill='x', expand=True, padx=10, pady=5)
...
def test_ortho4xp_single_tile():
    ...
    if not ok:
        if not messagebox.askyesno("Sync Warning", f"Could not sync settings to Ortho4XP config before test run:\n{info}\n\nThis may cause the test to use old settings. Continue anyway?"):
            return
    if ortho_override_step1_var.get():
    ...
    if ortho_apply_defaults_var.get(): write_ortho4xp_main_panel_settings(imagery=imagery, zl=zl, scenery_dir=dest_dir or None, custom_dem=final_custom_dem, custom_build_dir=dest_dir or None, dem_source=final_dem_source)
    elif final_custom_dem and os.path.exists(final_custom_dem):
        write_ortho4xp_main_panel_settings(custom_dem=final_custom_dem, dem_source=final_dem_source)

    write_ortho4xp_tile_config(cwd, tile_id, imagery, zl, final_custom_dem, dest_dir or None, dem_source=final_dem_source)
...
def run_ortho4xp_batch():
    ...
    if not ok:
        if not messagebox.askyesno("Sync Warning", f"Could not sync settings to Ortho4XP config before batch run:\n{info}\n\nThis may cause the batch to use old settings. Continue anyway?"):
            return
    if ortho_override_step1_var.get():
        write_ortho4xp_last_gui_params(S_LAT, S_LON, imagery, zl)
    ortho_dir = get_ortho4xp_dir()
    tiles = [(lat, lon) for lat in range(S_LAT, E_LAT + 1) for lon in range(S_LON, E_LON + 1)]
    ...
    if prov_ok is False and not messagebox.askyesno(
        ...
        return
    if len(tiles) > 25 and not messagebox.askyesno(
        "Confirm Large Batch",
        f"This will sequentially build {len(tiles)} tile(s) in Ortho4XP, one full Ortho4XP run per tile.\n"
        "This can take a very long time (hours to days depending on ZL). Continue?"
    ):
        return
    if ortho_apply_defaults_var.get():
        ok, info = write_ortho4xp_main_panel_settings(imagery=imagery, zl=zl, scenery_dir=dest_dir, custom_dem=custom_dem, custom_build_dir=dest_dir, ot_api_key=ot_api_key_var.get().strip(), dem_source=dem_source)
        if not ok:
            messagebox.showerror("Config Error", info); return
    elif custom_dem and os.path.exists(custom_dem):
        ok, info = write_ortho4xp_main_panel_settings(custom_dem=custom_dem, ot_api_key=ot_api_key_var.get().strip(), dem_source=dem_source)
        if not ok:
            messagebox.showerror("Config Error", info); return
    ortho4xp_is_paused = False; ortho4xp_stop_requested = False
    _load_ortho4xp_session()
    input_frame.pack_forget(); ortho_frame.pack_forget(); link_container.pack_forget(); dem_link_container.pack_forget()
    ortho_progress_frame.pack(fill='both', expand=True, padx=10, pady=20)
    threading.Thread(target=ortho4xp_worker_process, args=(tiles, exe_path, imagery, zl, custom_dem, dest_dir, dem_source), daemon=True).start()
...
def ortho4xp_worker_process(tiles, exe_path, imagery, zl, custom_dem, dest_dir, dem_source):
    ...
    for lat, lon in tiles:
        tile_id = _ortho4xp_tile_id(lat, lon)
        if ortho4xp_stop_requested:
            break
        while ortho4xp_is_paused and not ortho4xp_stop_requested:
    ...
    root.after(0, lambda: _handle_ortho4xp_worker_completion(completed, total, failed_tiles, crashed_tiles, ortho4xp_stop_requested, start_time))
...
def open_ortho4xp_global_settings_window():
    ...
    if not os.path.exists(cfg_path):
        if not messagebox.askyesno("Config Not Found", f"Ortho4XP.cfg not found at:\n{cfg_path}\n\nDo you want to create a new one with default values?"):
            return
            ...
            with open(cfg_path, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    ...
                    if not stripped_line or stripped_line.startswith('#') or '=' not in stripped_line:
                        continue
                    key, value = stripped_line.split('=', 1)
                    current_cfg[key.strip()] = value.strip().strip("'\"")
        except Exception as e:
            messagebox.showerror("Read Error", f"Failed to read config file:\n{e}", parent=settings_win)
    ...
    def save_and_sync_settings():
        try:
            ...
            # 2. Start with the configuration as it was when the window opened.
            ...
            for key in CANONICAL_KEYS:
                if key in final_cfg:
                    # Use the robust formatter to ensure valid output.
                    formatted_value = _format_cfg_value(key, final_cfg.get(key, ''))
                    new_content.append(f"{key}={formatted_value}\n")

            # 5. Write the complete, updated configuration back to the file.
            with open(cfg_path, 'w', encoding='utf-8') as f:
                f.writelines(new_content)
            ...
            messagebox.showerror("Save Error", f"Failed to write to config file:\n{e}", parent=settings_win)
...
def _handle_ortho4xp_worker_completion(completed, total, failed_tiles, crashed_tiles, was_stopped, start_time):
    if was_stopped:
        ortho_status_var.set("Status: Ortho4XP batch stopped by user.")
    else:
        elapsed = int(time.time() - start_time)
...
def toggle_mode_widgets(event=None):
    ...
    osmconvert_link_label.config(state=trig_state)
    osmfilter_link_label.config(state=trig_state)
    save_last_paths_checkpoint()
def browse_utils():
...
def open_osm_tools_folder():
    path = util_path_var.get().strip()
    if not path or not os.path.isdir(path):
        messagebox.showwarning("Folder Not Found", "The OSM Tools Folder path is not set or does not exist.")
        return
    try:
        if sys.platform == "win32":
...
def find_tool_in_dir(directory, tool_prefix):
    default_path = os.path.join(directory, f"{tool_prefix}.exe")
...
def _renumber_osm_ids(input_path, output_path):
    try:
        ...
        for elem in root:
            ...
            if elem.tag == 'node':
                id_map[old_id] = str(node_counter)
                node_counter -= 1
            elif elem.tag == 'way':
                id_map[old_id] = str(way_counter)
                way_counter -= 1
            elif elem.tag == 'relation':
                id_map[old_id] = str(relation_counter)
                relation_counter -= 1
        ...
        for elem in root:
            ...
            if elem.tag == 'way':
                for nd in elem.findall('nd'):
                    old_ref = nd.attrib.get('ref')
                    if old_ref in id_map:
                        nd.set('ref', id_map[old_ref])
            elif elem.tag == 'relation':
                for member in elem.findall('member'):
                    old_ref = member.attrib.get('ref')
                    if old_ref in id_map:
...
def bg_worker_process(START_LAT, END_LAT, START_LON, END_LON, util_dir, storage_dir, selected_file, osmconvert_path, osmfilter_path, resume_list, mode_selection):
    ...
    def process_single_tile_thread(worker_args):
        ...
        try:
            ...
            if not p_cut or p_cut.returncode != 0:
                output_text = ""
                try:
                    with open(log_cut_path, "r", encoding='utf-8', errors='replace') as f:
                        output_text = f.read().strip()
                except OSError:
                    pass
                print(f"[Tile {base_folder_name}] osmconvert (cut) failed. Code: {p_cut.returncode if p_cut else 'N/A'}. Output:\n{output_text}")
            ...
            filters = {
                ...
                "_small_roads.osm": 'highway=tertiary =tertiary_link =unclassified =residential =service',
            ...
            for suffix, query in filters.items():
                ...
                with open(p_file, "w", encoding="utf-8") as pf: pf.write(f"--keep={query}\n")
...
ortho_apply_defaults_var = tk.BooleanVar(value=True)
ortho_override_step1_var = tk.BooleanVar(value=False)
auto_run_ortho_var = tk.BooleanVar(value=False)
for var in (util_path_var, file_path_var, dest_path_var, ortho4xp_exe_var, ortho4xp_output_var, ot_api_key_var, ortho_imagery_var, ortho_zl_var, ortho_dem_choice_var, ortho_dem_custom_path_var, ortho_override_step1_var, ortho_apply_defaults_var, auto_run_ortho_var):
...
osmconvert_link_label = ttk.Label(input_frame, text="Click to download modern 64-bit osmconvert.exe", font=("Arial", 8, "underline"), foreground="#3498db", cursor="hand2")
osmconvert_link_label.grid(row=3, column=0, columnspan=3, sticky='w', padx=8)
osmconvert_link_label.bind("<Button-1>", open_osmconvert_link)
osmfilter_link_label = ttk.Label(input_frame, text="Click to download modern osmfilter.exe (pre-compiled)", font=("Arial", 8, "underline"), foreground="#3498db", cursor="hand2")
osmfilter_link_label.grid(row=4, column=0, columnspan=3, sticky='w', padx=8)
osmfilter_link_label.bind("<Button-1>", open_osmfilter_link)
ttk.Label(input_frame, text="Coordinates must be whole numbers (e.g., 27 or -35).", font=("Arial", 8, "italic")).grid(row=5, column=0, columnspan=3, pady=(8, 2), sticky='w')
ttk.Label(input_frame, text="Start Latitude (South):").grid(row=6, column=0, pady=4, sticky='w')
lat_s_entry = ttk.Entry(input_frame, width=15); lat_s_entry.grid(row=6, column=1, padx=10, pady=4, sticky='w')
ttk.Label(input_frame, text="End Latitude (North):").grid(row=7, column=0, pady=4, sticky='w')
lat_n_entry = ttk.Entry(input_frame, width=15); lat_n_entry.grid(row=7, column=1, padx=10, pady=4, sticky='w')
ttk.Label(input_frame, text="Start Longitude (West):").grid(row=8, column=0, pady=4, sticky='w')
lon_w_entry = ttk.Entry(input_frame, width=15); lon_w_entry.grid(row=8, column=1, padx=10, pady=4, sticky='w')
ttk.Label(input_frame, text="End Longitude (East):").grid(row=9, column=0, pady=4, sticky='w')
lon_e_entry = ttk.Entry(input_frame, width=15); lon_e_entry.grid(row=9, column=1, padx=10, pady=4, sticky='w')
ttk.Button(input_frame, text="\U0001F5FA Show Map Selector", command=open_map_window, width=20).grid(row=7, column=1, padx=145, pady=4, sticky='w')
ttk.Label(input_frame, text="OSM Tools Folder:").grid(row=10, column=0, pady=6, sticky='w')
util_entry = ttk.Entry(input_frame, textvariable=util_path_var, width=43); util_entry.grid(row=10, column=1, padx=10, pady=6, sticky='ew')
util_btn_frame = ttk.Frame(input_frame)
util_btn_frame.grid(row=10, column=2, padx=(6, 0), pady=6, sticky='w')
ttk.Button(util_btn_frame, text="Browse...", command=browse_utils, width=8).pack(side='left')
ttk.Button(util_btn_frame, text="Open", command=open_osm_tools_folder, width=6).pack(side='left', padx=(4, 0))
ttk.Label(input_frame, text="Master OSM Data File (.pbf):").grid(row=11, column=0, pady=6, sticky='w')
pbf_entry = ttk.Entry(input_frame, textvariable=file_path_var, width=43); pbf_entry.grid(row=11, column=1, padx=10, pady=6, sticky='w')
pbf_btn = ttk.Button(input_frame, text="Browse...", command=browse_file, width=10); pbf_btn.grid(row=11, column=2, padx=(6, 0), pady=6, sticky='w')
ttk.Label(input_frame, text="OSM Tile Data Output Folder:").grid(row=12, column=0, pady=6, sticky='w')
ttk.Entry(input_frame, textvariable=dest_path_var, width=43).grid(row=12, column=1, padx=10, pady=6, sticky='w')
dest_btn = ttk.Button(input_frame, text="Browse...", command=browse_destination, width=10); dest_btn.grid(row=12, column=2, padx=(6, 0), pady=6, sticky='w')

ortho_frame = ttk.LabelFrame(scrollable_frame, text="Step 2: Ortho4XP Integration")
ortho_frame.pack(fill='x', expand=True, padx=10, pady=5)
ortho_frame.grid_columnconfigure(1, weight=1)
ortho_frame.grid_columnconfigure(2, weight=0)

ttk.Label(ortho_frame, text="Ortho4XP.exe Location:").grid(row=1, column=0, padx=8, pady=4, sticky='w')
ortho_exe_entry = ttk.Entry(ortho_frame, textvariable=ortho4xp_exe_var, width=43); ortho_exe_entry.grid(row=1, column=1, padx=10, pady=4, sticky='w')
ortho_exe_btn_frame = ttk.Frame(ortho_frame)
ortho_exe_btn_frame.grid(row=1, column=2, padx=(6, 0), pady=4, sticky='w')
ttk.Button(ortho_exe_btn_frame, text="Browse...", command=browse_ortho4xp_exe, width=10).pack(side='left')
ttk.Button(ortho_exe_btn_frame, text="Sync Now", command=manual_sync_with_ortho4xp, width=10).pack(side='left', padx=(4,0))
ttk.Button(ortho_exe_btn_frame, text="Global Settings", command=open_ortho4xp_global_settings_window, width=15).pack(side='left', padx=(4,0))
ttk.Label(ortho_frame, text="Ortho4XP Tile Output Folder: (Required for Batch)").grid(row=2, column=0, padx=8, pady=4, sticky='w')
ortho_output_entry = ttk.Entry(ortho_frame, textvariable=ortho4xp_output_var, width=43); ortho_output_entry.grid(row=2, column=1, padx=10, pady=4, sticky='w')
ttk.Button(ortho_frame, text="Browse...", command=browse_ortho4xp_output, width=10).grid(row=2, column=2, padx=(6, 0), pady=4, sticky='w')
ttk.Label(ortho_frame, text="Finished Ortho4XP tiles will be moved here. Defaults to Ortho4XP's current setting.", font=("Arial", 8, "italic")).grid(row=3, column=0, columnspan=2, padx=8, pady=(0, 4), sticky='w')
ttk.Label(ortho_frame, text="Imagery Provider:").grid(row=4, column=0, padx=8, pady=4, sticky='w');
ortho_imagery_combo = ttk.Combobox(ortho_frame, textvariable=ortho_imagery_var, width=15); ortho_imagery_combo.grid(row=4, column=1, padx=10, pady=4, sticky='w')
ttk.Label(ortho_frame, text="e.g. BI=Bing, GO2=Google. Or type your own provider code.", font=("Arial", 8, "italic")).grid(row=5, column=0, columnspan=2, padx=8, pady=(0, 4), sticky='w')
ttk.Label(ortho_frame, text="Zoom Level (ZL):").grid(row=6, column=0, padx=8, pady=4, sticky='w')
ortho_zl_combo = ttk.Combobox(ortho_frame, textvariable=ortho_zl_var, values=ORTHO4XP_ZL_CHOICES, width=15); ortho_zl_combo.grid(row=6, column=1, padx=10, pady=4, sticky='w')
ttk.Label(ortho_frame, text="Elevation Data Source (DEM):").grid(row=7, column=0, padx=8, pady=4, sticky='w')
ortho_dem_combo = ttk.Combobox(ortho_frame, textvariable=ortho_dem_choice_var, values=ORTHO4XP_DEM_CHOICES, width=25, state='readonly'); ortho_dem_combo.grid(row=7, column=1, padx=10, pady=4, sticky='w')
ortho_dem_combo.bind("<<ComboboxSelected>>", on_dem_choice_change)
ortho_dem_path_label = ttk.Label(ortho_frame, text="(using Ortho4XP's default elevation data)", font=("Arial", 8, "italic"))
ortho_dem_path_label.grid(row=8, column=0, columnspan=2, padx=8, pady=(0, 4), sticky='w')

ttk.Label(ortho_frame, text="OpenTopography API Key:").grid(row=9, column=0, padx=8, pady=4, sticky='w')
ot_api_key_entry = ttk.Entry(ortho_frame, textvariable=ot_api_key_var, width=43)
ot_api_key_entry.grid(row=9, column=1, padx=10, pady=4, sticky='w')

ortho_cfg_options_frame = ttk.Frame(ortho_frame)
ortho_cfg_options_frame.grid(row=10, column=0, columnspan=3, sticky='w', padx=4, pady=2)
ttk.Checkbutton(ortho_cfg_options_frame, text="Sync these settings to Ortho4XP's main panel (Ortho4XP.cfg)", variable=ortho_apply_defaults_var).pack(anchor='w')
ttk.Checkbutton(ortho_cfg_options_frame, text="Remember last tile for manual Ortho4XP launch", variable=ortho_override_step1_var).pack(anchor='w')
ttk.Label(ortho_frame, text="This tool automates Ortho4XP. Use 'Step 1' for standalone OSM data.", font=("Arial", 8, "italic")).grid(row=11, column=0, columnspan=3, padx=8, pady=(4, 6), sticky='w')
ortho_btn_holder = ttk.Frame(ortho_frame); ortho_btn_holder.grid(row=12, column=0, columnspan=3, padx=8, pady=8, sticky='w')
ttk.Button(ortho_btn_holder, text="Test Single Tile", command=test_ortho4xp_single_tile, width=16).pack(side='left', padx=(0, 8))
ttk.Button(ortho_btn_holder, text="Build Batch", command=run_ortho4xp_batch, style="Success.TButton").pack(side='left', padx=(0, 8))
ttk.Button(ortho_btn_holder, text="Clean Tiles", command=cleanup_selected_tile_batch, style="Danger.TButton").pack(side='left', padx=(0, 8))
...
ttk.Button(ortho_btn_container, text="Stop Ortho4XP Batch", command=trigger_ortho4xp_stop, style="Danger.TButton", width=25).pack(side='left', padx=10)

ttk.Checkbutton(input_frame, text="Automatically start Step 2 (Ortho4XP Batch) after Step 1 completes", variable=auto_run_ortho_var).grid(row=13, column=0, columnspan=3, sticky='w', padx=4, pady=(6,0))
...
ot_api_key_link_label = ttk.Label(dem_link_container, text="Get an API key for automatic OpenTopography DEM downloads", font=("Arial", 9, "underline"), foreground="#3498db", cursor="hand2")
ot_api_key_link_label.pack(anchor='w')
ot_api_key_link_label.bind("<Button-1>", open_ot_api_key_link)
...
update_ortho4xp_providers_list()
if saved_state and saved_state.get("ortho4xp_imagery"):
    ortho_imagery_var.set(saved_state.get("ortho4xp_imagery"))

ensure_ortho4xp_cfg_synced()
