    ...
    def process_single_tile_thread(worker_args):
        ...
        try:
            ...
            try:
                ...
                print(f"[Tile {base_folder_name}] Error during osmconvert (cut): {e}")
                # Even with an error, we ensure a dummy file exists to allow processing to continue for empty tiles.

            # For sea-only tiles, osmconvert might not produce an output file and may exit with an error.
            # We handle this by creating a dummy empty OSM file if one doesn't exist.
            # This allows the rest of the pipeline to correctly generate empty .bz2 files.
            if not os.path.exists(loc_tile) or os.path.getsize(loc_tile) == 0:
                try:
                    ...
                    print(f"[Tile {base_folder_name}] CRITICAL: Failed to create dummy tile file after osmconvert failure: {file_err}")
